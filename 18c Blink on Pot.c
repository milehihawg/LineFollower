...
/*
Based on the PotVal the LED will blink faster or slower.
Loop Forever
Grab PotVal
Blink waitinMilliseconds based on PotVal

Blink cycle is 4093/1000MS = 4sec. (4sec on and 4sec off)
8 seconds / 4 = 2 second cycle
*/
int PotVal;
task main()
{
	while (1==1)
	{
		PotVal=SensorValue[Pot]/4;
		turnLEDOn(LEDRed);
		waitInMilliseconds(PotVal);
		turnLEDOff(LEDRed);
		waitInMilliseconds(PotVal);
